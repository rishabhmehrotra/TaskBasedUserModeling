...
//		System.out.println("");
	...
	private double[] getTopicProportions() {
	    double[] props = new double[numTopics];
	    
	    for(int d = 0; d < data.size(); d++) {
	        double[] docTopics = getTopicProbabilities(d);
	    ...
	    for(int k = 0; k < numTopics; k++) {
            props[k] = props[k] / (double)data.size();
        }
	    
	    return props;
	}

	public String displayTopWords (int numWords, boolean usingNewLines) throws IOException {
		...
		double[] topicProportions = getTopicProportions();
		
		DecimalFormat dformatter = new DecimalFormat("0.00000");
		...
		for (int topic = 0; topic < numTopics; topic++) {
			...
			if (usingNewLines) {
				out.append (String.format("%02d", topic) + "  " + dformatter.format(topicProportions[topic]) + "  " + dformatter.format(alpha[topic]) + "    ");
				try {
					out1.write (String.format("%02d", topic) + "  " + dformatter.format(topicProportions[topic]) + "  " + dformatter.format(alpha[topic]) + "    ");
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				while (iterator.hasNext() && word < numWords) {
					...
					try {
						out1.append(alphabet.lookupObject(info.getID()) + "\t" + formatter.format(info.getWeight()) + "\n");
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				out.append (String.format("%02d", topic) + "  " + dformatter.format(topicProportions[topic]) + "  " + dformatter.format(alpha[topic]) + "    ");
				try {
					out1.append (String.format("%02d", topic) + "  " + dformatter.format(topicProportions[topic]) + "  " + dformatter.format(alpha[topic]) + "    ");
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				...
				while (iterator.hasNext() && word < numWords) {
					...
					try {
						out1.append(alphabet.lookupObject(info.getID()) + " ");
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
